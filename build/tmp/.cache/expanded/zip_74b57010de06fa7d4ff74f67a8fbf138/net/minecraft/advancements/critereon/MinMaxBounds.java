...
public interface MinMaxBounds<T extends Number> {
    ...
    static <T extends Number, R extends MinMaxBounds<T>> Codec<R> createCodec(Codec<T> p_297837_, MinMaxBounds.BoundsFactory<T, R> p_298619_) {
        Codec<R> codec = RecordCodecBuilder.create(
            p_325235_ -> p_325235_.group(
                    p_297837_.optionalFieldOf("min").forGetter(MinMaxBounds::min), p_297837_.optionalFieldOf("max").forGetter(MinMaxBounds::max)
                )
    ...
    static <B extends ByteBuf, T extends Number, R extends MinMaxBounds<T>> StreamCodec<B, R> createStreamCodec(
        final StreamCodec<B, T> p_391981_, final MinMaxBounds.BoundsFactory<T, R> p_396126_
    ) {
        return new StreamCodec<B, R>() {
            private static final int MIN_FLAG = 1;
            public static final int MAX_FLAG = 2;

            public R decode(B p_393437_) {
                byte b0 = p_393437_.readByte();
                Optional<T> optional = (b0 & 1) != 0 ? Optional.of(p_391981_.decode(p_393437_)) : Optional.empty();
                Optional<T> optional1 = (b0 & 2) != 0 ? Optional.of(p_391981_.decode(p_393437_)) : Optional.empty();
                return p_396126_.create(optional, optional1);
            }

            public void encode(B p_392435_, R p_396875_) {
                Optional<T> optional = p_396875_.min();
                Optional<T> optional1 = p_396875_.max();
                p_392435_.writeByte((optional.isPresent() ? 1 : 0) | (optional1.isPresent() ? 2 : 0));
                optional.ifPresent(p_397001_ -> p_391981_.encode(p_392435_, (T)p_397001_));
                optional1.ifPresent(p_393724_ -> p_391981_.encode(p_392435_, (T)p_393724_));
            }
        };
    }
    ...
    public record Ints(Optional<Integer> min, Optional<Integer> max, Optional<Long> minSq, Optional<Long> maxSq)
