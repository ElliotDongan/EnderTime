...
public class BreezeDebugRenderer {
    ...
    public void render(PoseStack p_311387_, MultiBufferSource p_310722_, double p_312623_, double p_310151_, double p_312438_) {
        ...
        localplayer.level()
            ...
            .forEach(
                p_405018_ -> {
                    Optional<BreezeDebugPayload.BreezeInfo> optional = Optional.ofNullable(this.perEntity.get(p_405018_.getId()));
                    optional.map(BreezeDebugPayload.BreezeInfo::attackTarget)
                        .map(p_405020_ -> localplayer.level().getEntity(p_405020_))
                        .map(p_357967_ -> p_357967_.getPosition(this.minecraft.getDeltaTracker().getGameTimeDeltaPartialTick(true)))
                        .ifPresent(
                            p_310972_ -> {
                                drawLine(p_311387_, p_310722_, p_312623_, p_310151_, p_312438_, p_405018_.position(), p_310972_, TARGET_LINE_COLOR);
                    ...
                    optional.map(BreezeDebugPayload.BreezeInfo::jumpTarget)
                        .ifPresent(
                            p_405011_ -> {
                                drawLine(p_311387_, p_310722_, p_312623_, p_310151_, p_312438_, p_405018_.position(), p_405011_.getCenter(), JUMP_TARGET_LINE_COLOR);
                                DebugRenderer.renderFilledBox(
                                    p_311387_,
                                    p_310722_,
                                    AABB.unitCubeFromLowerCorner(Vec3.atLowerCornerOf(p_405011_)).move(-p_312623_, -p_310151_, -p_312438_),
                                    1.0F,
                                    0.0F,
                                    0.0F,
                                    1.0F
